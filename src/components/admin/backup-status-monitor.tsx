...
import { useBackupRealtime } from '@/lib/realtime-provider';
import { 
  Database, 
  RefreshCw, 
  CheckCircle, 
...
export function BackupStatusMonitor() {
  ...
  const realtimeData = useBackupRealtime();
  const { 
  ...
  const backupProgress = (realtimeData as any).backupProgress || { current: 0, total: 0 };

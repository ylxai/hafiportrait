...
export type AlertChannelType = 'slack' | 'email' | 'sms' | 'webhook' | 'push' | 'whatsapp';
...
class AlertManager {
  ...
  private async sendNotifications(alert: Alert, escalationRule: EscalationRule): Promise<void> {
    const promises = escalationRule.channels.map(async (channelType: AlertChannelType) => {
  ...
  async resolveAlert(alertId: string, resolvedBy: string): Promise<boolean> {
    ...
    keysToDelete.forEach(key => this.escalationTimers.delete(key));

...
const envSchema = z.object({
  ...
  // Railway Upload API
  NEXT_PUBLIC_UPLOAD_API_URL: z
    .string()
    .url('NEXT_PUBLIC_UPLOAD_API_URL must be a valid URL'),
  NEXT_PUBLIC_UPLOAD_API_KEY: z
    .string()
    .min(1, 'NEXT_PUBLIC_UPLOAD_API_KEY is required'),
...
function validateEnv(): Env {
  try {
    ...
    if (parsed.NODE_ENV === 'production') {
      if (
        parsed.NEXTAUTH_SECRET ===
        'generate-a-secure-secret-minimum-32-characters-long'
      ) {
        throw new Error(
          'CRITICAL: Default NEXTAUTH_SECRET detected in production! Generate a secure secret.'
        )
      }
